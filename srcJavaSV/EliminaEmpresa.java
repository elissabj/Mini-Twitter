 ...
 */
@WebServlet(name = "EliminaEmpresa", urlPatterns = {"/EliminaEmpresa"})
public class EliminaEmpresa extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try {
                BaseDatos.conectar();
                rsValida = BaseDatos.consulta("call sp_BajaEmpresa('" + idAdmon + "','" + nomAdmon + "','" + idPersona + "','" + NombreEmpresa + "')");

                while (rsValida.next()) {
                    if (rsValida.getString("Respuesta").equalsIgnoreCase("Empresa Eliminada")) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet EliminaEmpresa</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet EliminaEmpresa at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }

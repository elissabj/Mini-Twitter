 ...
 */
@WebServlet(name = "EliminaSeguidor", urlPatterns = {"/EliminaSeguidor"})
public class EliminaSeguidor extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try {
                BaseDatos.conectar();
                rsValida = BaseDatos.consulta("call sp_BajaSeguidosEmpresa('"+IdPersonaMaster+"','"+NombreMaster+"','"+idSeguido+"','"+NombreSeguido+"');");
                
                while (rsValida.next()) {                    
                    if(rsValida.getString("Respuesta").equalsIgnoreCase("Persona Inexistente")){
                        response.sendRedirect("MenuEmpresa.jsp");
                    }else if(rsValida.getString("Respuesta").equalsIgnoreCase("Seguidor Eliminado")){
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet EliminaSeguidor</title>");            
            out.println("</head>");
            out.println("<body>");

 ...
 */
@WebServlet(name = "RegistraForo", urlPatterns = {"/RegistraForo"})
public class RegistraForo extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try {
                BaseDatos.conectar();
                rsValida = BaseDatos.consulta("call sp_AltaForo('"+Titulo+"','"+Contenido+"','"+idUsuario+"','"+NombreUsr+"');");
                
                while (rsValida.next()) {                    
                    if(rsValida.getString("Respuesta").equalsIgnoreCase("Foro Existente")){
                        response.sendRedirect("MenuEmpresa.jsp");
                    }else if(rsValida.getString("Respuesta").equalsIgnoreCase("Foro Registrado")){
